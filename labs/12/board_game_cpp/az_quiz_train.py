...
parser.add_argument("--evaluate_each", default=2, type=int, help="Evaluate each number of iterations.")
parser.add_argument("--learning_rate", default=0.001, type=float, help="Learning rate.")
parser.add_argument("--model_path", default="models/az_quiz_alpha0.15_lr0.001_sim750_sample10.pt", type=str, help="Model path")
parser.add_argument("--num_simulations", default=750, type=int, help="Number of simulations in one MCTS.")
...
class Agent:
    ...
    def board_features(self, game: AZQuiz) -> np.ndarray:
        board = np.zeros((AZQuiz.N, AZQuiz.N, AZQuiz.C), dtype=np.float32)

        for y in range(AZQuiz.N):
            for x in range(AZQuiz.N):
                field = game.board[y, x]

                board[y, x, 0] = field == 2 + game.to_play
                board[y, x, 1] = field == 3 - game.to_play
                board[y, x, 2] = field == 1
                board[y, x, 3] = field >= 0
...
# Rotate the board 
# -------------
def azquiz_action_coordinates():
    # Obtain valid loacation in the board
    # [
    # (0, 0),
    # (1, 0), (1, 1),
    # (2, 0), (2, 1), (2, 2),
    # ...
    return [(y, x) for y in range(AZQuiz.N) for x in range(y + 1)]

# Map actions to coordinations (if you know action this enables you to get its coords)
AZQUIZ_ACTION_TO_COORD = azquiz_action_coordinates()

# coordinations to actions
AZQUIZ_COORD_TO_ACTION = {coord: action for action, coord in enumerate(AZQUIZ_ACTION_TO_COORD)}

def rotate_coord_120(y: int, x: int) -> tuple[int, int]:
    # Rotate triangular AZQuiz board coordinates by 120 degrees.
    n = AZQuiz.N
    new_y = n - 1 - y + x
    new_x = n - 1 - y
    return new_y, new_x

def rotate_board_and_policy_once(
    board: np.ndarray,
    policy: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    # Rotate board and policy by 120 degrees.
    rotated_board = np.zeros_like(board)
    rotated_policy = np.zeros_like(policy)

    for action, (y, x) in enumerate(AZQUIZ_ACTION_TO_COORD):
        new_y, new_x = rotate_coord_120(y, x)
        new_action = AZQUIZ_COORD_TO_ACTION[(new_y, new_x)]

        rotated_board[new_y, new_x, :] = board[y, x, :]
        rotated_policy[new_action] = policy[action]

    return rotated_board, rotated_policy

def augment_board_and_policy(
    board: np.ndarray,
    policy: np.ndarray,
) -> list[tuple[np.ndarray, np.ndarray]]:
    """
    Return original board/policy plus two rotational symmetries.
    """
    board0 = board
    policy0 = policy

    board1, policy1 = rotate_board_and_policy_once(board0, policy0)
    board2, policy2 = rotate_board_and_policy_once(board1, policy1)
